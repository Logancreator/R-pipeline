...
cd /md01/changjy/data/Goose/Goose_bulk/map
ls *.sort.bam|while read line
do
bam_file=$line
out=${bam_file%.bam*}.q30.unchrM.sam
echo "----------------------------------"
echo "sam_file is : $bam_file"
echo "extract unChrM.sam at $(date +%F%n%T)"
#awk '$3!="NC_023832.1"|| NF<10' $bam_file |samtools view -S -b -f 0x2 -q 10 |samtools sort -o   > $out
~/software/miniconda2/envs/atac/bin/samtools view  -q 30  -f 0x2 $bam_file|awk '$3!="NC_023832.1"' > $out
echo "extract unChrM  finished ... at $(date +%F%n%T)"
done

#transfer bam and q10 sort
ls *.unchrM.sam|while read line
do
unchrM_file=$line
out=${unchrM_file%.*}
echo "sam_file is : $sam_file"
...
echo "extract unChrM.sam at $(date +%F%n%T)"
~/software/miniconda2/envs/atac/bin/samtools view -S -b -f 0x2 -q 10 $unchrM_file|~/software/miniconda2/envs/atac/bin/samtools sort -o $out.pe.q30.sort.bam
echo "extract unChrM  finished ... at $(date +%F%n%T)"
echo "----------------------------------"
echo "\n"
echo "\n"
...
###构建对比集
dba_contrast1 <- dba.contrast(dba_count1, categories=DBA_CONDITION,minMembers=4)
...
y=read.csv("C:/Users/Jeff/OneDrive/桌面/motif.csv")
library(ggplot2)
...
pfm <- PFMatrix(ID="MA0004.1", name=motif, 
...
cd /md01/shipy3/software/gffread
tar -xzvf gffread-0.12.7.Linux_x86_64.tar.gz
module load gffread

## Preparing transcriptome indices 
# Extracting transcript sequences
# reference : http://ccb.jhu.edu/software/stringtie/gff.shtml#gffread
#gffread -w /public/home/shipy3/DB/Smed/smedSxl/fasta/smedSxl_transcripts.fa -g /public/home/shipy3/DB/Smed/smedSxl/fasta/smedSxlgenome.fa /public/home/shipy3/DB/Smed/smedSxl/annotation/smedSxlgenes_exon.gtf
fa="GCF_001577835.2_Coturnix_japonica_2.1_genomic.fna"
trans="rename_GCF_001577835.2_Coturnix_japonica_2.1_rna_from_genomic.fna"
# Selective Alignment
# reference : https://combine-lab.github.io/alevin-tutorial/2019/selective-alignment/
#  extract the names of the genome targets
#grep "^>" <(gunzip -c $fa) | cut -d " " -f 1 > decoys.txt
grep "^>"  $fa | cut -d " " -f 1 > decoys.txt
sed -i.bak -e 's/>//g' decoys.txt
# Along with the list of decoys salmon also needs the concatenated transcriptome and genome reference file for index
cat $trans $fa |gzip > gentrome.fa.gz
# Salmon Indexing
/md01/changjy/software/salmon-1.5.2_linux_x86_64/bin/./salmon index -t gentrome.fa.gz -d decoys.txt -p 12 -i salmon_index



# 组成型增强子排序
bedtools sort -i SD28_LD3_LD7_LD28_peak_merged.txt >quail.enhancer.sort.bed

# 距离在12.5 kb内的增强子归为一簇
bedtools cluster -d 125000 -i quail.enhancer.sort.bed >quail.cluster.enhancer.bed

# 计算每个增强子的H3K27ac结合强度
bedtools coverage -c -a quail.cluster.enhancer.bed -b SD28.bam \
>SD28_quail.cluster.enhancer.H3K27ac.profile_tmp

bedtools coverage -c -a quail.cluster.enhancer.bed -b LD3.bam \
>LD3_quail.cluster.enhancer.H3K27ac.profile_tmp

bedtools coverage -c -a quail.cluster.enhancer.bed -b LD7.bam \
>LD7_quail.cluster.enhancer.H3K27ac.profile_tmp

bedtools coverage -c -a quail.cluster.enhancer.bed -b LD28.bam \
>LD28_quail.cluster.enhancer.H3K27ac.profile_tmp


# 对每簇增强子的结合强度做簇内加和
# 注意： -g 指定以那一列分组，指定的应该是标记分簇的数字所在的列;
# -c 表示对coverage所在的列计算加和 (-o sum)，注意列需要根据实际指定
bedtools groupby -i SD28_quail.cluster.enhancer.H3K27ac.profile_tmp -g 4 -c 5 -o sum \
>SD28_quail.cluster.enhancer.H3K27ac.profile

bedtools groupby -i LD3_quail.cluster.enhancer.H3K27ac.profile_tmp -g 4 -c 5 -o sum \
>LD3_quail.cluster.enhancer.H3K27ac.profile

bedtools groupby -i LD7_quail.cluster.enhancer.H3K27ac.profile_tmp -g 4 -c 5 -o sum \
>LD7_quail.cluster.enhancer.H3K27ac.profile

bedtools groupby -i LD28_quail.cluster.enhancer.H3K27ac.profile_tmp -g 4 -c 5 -o sum \
>LD28_quail.cluster.enhancer.H3K27ac.profile


#以下为R代码请在R中运行

#####以下为R代码
for (i in c("SD28","LD3","LD7","LD28")){
  enhancer_input=paste(i,"_quail.cluster.enhancer.H3K27ac.profile",sep="")

  bed_input=paste(i,"_quail.cluster.enhancer.H3K27ac.profile_tmp",sep="")
  
  pdf_name=paste(i,"_super_enhancer_line.pdf",sep="")
  
  enhancer = read.table(enhancer_input,header=F, row.names=NULL, sep="\t")

  bed = read.table(bed_input,header=F, row.names=NULL, sep="\t")
  ...
  abline(v= xPt,h= y_cutoff,lty=2,col=8,lwd=5)
  points(xPt,y_cutoff,pch=16,cex=0.9,col=2)
  abline(coef=c(b,slope),col=2,lwd=5)
  title(paste("x=",xPt,"\ny=",signif(y_cutoff,3),"\nFold over Median=",
  ...
  ## 超级增强子cluster
  print(enhancer[enhancer$V2>=y_cutoff,1])
  print(length(enhancer[enhancer$V2>=y_cutoff,1]))
  
  # super_enhancer_peakname_output=paste(i,"_quail_super_enhancer_peakname_output.txt",sep="")

  # super_enhancer=bed[bed$V4%in%enhancer[enhancer$V2>=y_cutoff,1],]
  # super_enhancer_output=paste(i,"_super_enhancer_cluster_peak.txt",sep="")
  # write.table(super_enhancer,super_enhancer_output,sep="\t",quote=F,row.names=F,col.names=F)
  # write.table(enhancer[enhancer$V2>=y_cutoff,1],super_enhancer_peakname_output,sep="\t",quote=F,row.names=F,col.names=F)
}


bed_SD28=read.table("SD28_quail_super_enhancer_peakname_output.txt",,sep="\t")
bed_LD3=read.table("LD3_quail_super_enhancer_peakname_output.txt",,sep="\t")
bed_LD7=read.table("LD7_quail_super_enhancer_peakname_output.txt",,sep="\t")
bed_LD28=read.table("LD28_quail_super_enhancer_peakname_output.txt",,sep="\t")
setdiff(bed_SD28$V1,bed_LD3$V1)
setdiff(bed_SD28$V1,bed_LD7$V1)
setdiff(bed_SD28$V1,bed_LD28$V1)
setdiff(bed_LD3$V1,bed_LD7$V1)
setdiff(bed_LD7$V1,bed_LD28$V1)

bed=read.table("quail.cluster.enhancer.bed")
bed_SD28_vs_LD3=bed[bed$V4%in%c(31,37,141),]
bed_SD28_vs_LD7=bed[bed$V4%in%c(3,389),]
write.table(bed_SD28_vs_LD3,"bed_SD28_vs_LD3.txt",sep="\t",row.names=F,col.names=F,quote=F)
write.table(bed_SD28_vs_LD7,"bed_SD28_vs_LD7.txt",sep="\t",row.names=F,col.names=F,quote=F)


data1=read.csv("/md01/changjy/data/online_data/RNA-seq/mydata_count_rpkm.txt",sep="\t")
data2=read.csv("/md01/changjy/data/online_data/RNA-seq/online_count_rpkm.txt",sep="\t")
anno=as.character(unique(read.csv("bed_SD28_vs_LD3_peakAnno.txt",sep="\t",header=F)[,13]))
#anno=as.character(unique(read.csv("bed_SD28_vs_LD7_peakAnno.txt",sep="\t",header=F)[,13]))
use1=data1[data1$X%in%anno,]
#use2=data2[data2$X%in%anno,]
use1$ccc=rep("use1",dim(use1)[1])
#use2$ccc=rep("use2",dim(use2)[1])
#boxplot(use1$log2FoldChange)

library(ggplot2)
ggplot(use1, aes(x=ccc, y=log2FoldChange)) + 
  geom_boxplot()
...
library(ggplot2)
library(pheatmap)
data=read.csv("C:\\Users\\Jeff\\OneDrive\\桌面\\Gene(1).csv",row.names = 1,header=T)[-12,]
