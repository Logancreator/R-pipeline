...
##Corralation计算
	...
	rlogMat <- assay(rld)
	rlogMat <- rlogMat[order(normalized_counts, decreasing=T), ]
	...
	pdf("ATAC.DESeq2.normalized.rlog.pearson.pdf", pointsize=10)
	heatmap.2(pearson_cor, Rowv=as.dendrogram(hc), symm=T, trace="none",
	col=hmcol, dendrogram="none",margins=c(11,11), main="The pearson correlation",,rowV="as-is",rowInd=c(
							"L2P1_L1_809F10",
                            "L2P2_L1_810F10",
                            "L5P1_L1_811F10",
                            "L5P2_L1_812F10",
...
##read data
data=read.csv("/public/home/changjianye/project/scatac/Anser_cygnoides/genes/maker_list.csv",
	header=T,row.names=1)
type=unique(data$cluster)
...
for (i in c(1:length(type))){
	markerGenes=data[data$cluster%in%type[i],]
	markerGenes=markerGenes[markerGenes$gene%in%as.data.frame(genes)$symbol,'gene']
	p <- plotEmbedding(
	    ArchRProj = projHeme2, 
	    colorBy = "GeneScoreMatrix", 
	    name =  as.character(markerGenes), 
	    embedding = "UMAP2",
	    imputeWeights = getImputeWeights(projHeme2)
	)
	plotPDF(plotList = p, 
	    name = paste("Plot-UMAP-Marker-Genes-",type[i],"-Imputation.pdf",sep=""), 
	    ArchRProj = projHeme2, 
...
projHeme2 <- addIterativeLSI( ArchRProj = projHeme2,useMatrix = "TileMatrix",name = "IterativeLSI3", iterations = 2, 
  clusterParams = list(resolution = c(1),sampleCells = length(projHeme2$cellNames), n.start = 10), force=TRUE,
  varFeatures = 30000, dimsToUse = 1:15)

####add cluster####
projHeme2 <- addClusters(input = projHeme2, reducedDims = "IterativeLSI3",name = "Clusters2", force = TRUE, resolution = 1,dimsToUse = 1:15)
table(projHeme2$Clusters2)

projHeme2 <- addUMAP(ArchRProj = projHeme2, reducedDims = "IterativeLSI3", nNeighbors = 10, minDist = 0.3, metric = "cosine", force = TRUE)
...
plotPDF(p1,p2, name = "Plot-UMAP2-Sample-Clusters.pdf", ArchRProj = projHeme2, addDOC = FALSE, width = 5, height = 5)
p1 <- plotEmbedding(ArchRProj = projHeme2, colorBy = "cellColData", name = "Sample", embedding = "UMAP2")
p2 <- plotEmbedding(ArchRProj = projHeme2, colorBy = "cellColData", name = "ScranClusters2", embedding = "UMAP2")
ggAlignPlots(p1, p2, type = "h")
plotPDF(p1,p2, name = "Plot-UMAP-Sample-Scran2Clusters.pdf", ArchRProj = projHeme2, addDOC = FALSE, width = 5, height = 5)





p <- plotEmbedding(ArchRProj = projHeme2,colorBy = "GeneScoreMatrix",name = c("FSHB","TBX19","HES1","ISL1","PRL","TCIM","IRF4","TAL1","CHGA"),embedding = "UMAP2",
        imputeWeights = getImputeWeights(projHeme2))
...
idxPass <- which(projHeme2$TSSEnrichment >= 1.5 & log10(projHeme2$nFrags) >=3.5)
...
library(dplyr)
library(ggplot2)
anno=read.csv("/md01/changjy/data/all/quail_diff/TF/cluster3/position/all.anno",sep="\t",header=F)
#rna=read.csv("/md01/changjy/data/online_data/RNA-seq/Count_matrix_rpkm.csv",header=T) LD28
rna=read.csv("/md01/changjy/data/online_data/RNA-seq/",header=T) ##LD7
data=rna[(rna$X%in%anno$V17)&(rna$SD28_mean>=1|rna$LD28_mean>=1), ] 
data=arrange(data,data$log2FoldChange)
data$X=factor(data$X,levels=data$X)

data[data$log2FoldChange>=0,"color"]="Up"
data[data$log2FoldChange< 0,"color"]="Down"

ggplot(data=data,mapping=aes(x=X,y=log2FoldChange,fill=color,group=factor(1)))+
  geom_bar(stat="identity",width=0.85)+theme(text = element_text(size=10),
  ...
  scale_fill_manual(values=c("lightblue","#EE6363"))+xlab(NULL)+ coord_flip()
  ggsave("/md01/changjy/data/all/quail_diff/TF/cluster3/position/test.pdf")
...
idxSample <- BiocGenerics::which(projHeme2$Sample %in% "3E-ME")
cellsSample <- projHeme2$cellNames[idxSample]
ME3=projHeme2[cellsSample, ]


idxPass <- which(log10(ME3$nFrags) >= 3.6)
cellsPass <- ME3$cellNames[idxPass]
ME3=ME3[cellsPass, ]
