...
--id=1E-P \
--reference=/md01/changjy/data/Goose_1.0/ref/Anser_cygnoides_chr \
...
##sort tsv file
#sort -k1,1 -k2,2n new.fragments.tsv  > fragments.resorted.tsv
##compress the tsv file
bgzip new.fragments.tsv 
#make the index for tsv.gz file
tabix -p bed new.fragments.tsv 
...
projHemeNew<-loadArchRProject("/md01/changjy/data/Goose_1.0/snATAC_ME")
projHemeNew<-loadArchRProject("/md01/changjy/data/Goose_1.0/snATAC_PT")
======================================================================================================
...
genes<-read.csv("/md01/changjy/data/Goose_1.0/ref/Anser_cygnoides_chr/rename_sorted.genes.txt",sep="\t")
TSS<-read.csv("/md01/changjy/data/Goose_1.0/ref/Anser_cygnoides_chr/rename_sorted.TSS.txt",sep="\t")
exons<-read.csv("/md01/changjy/data/Goose_1.0/ref/Anser_cygnoides_chr/rename_sorted.exons.txt",sep="\t")
...
library(BSgenome.Acygnoides.Assembly.go12)
chromsize<-read.table("/md01/changjy/data/Goose_1.0/ref/Anser_cygnoides_chr/fasta/genome.fa.fai")
...
projHemeNew <- ArchRProject(ArrowFiles = ArrowFiles,
                         outputDirectory = "/md01/changjy/data/Goose_1.0/snATAC_ME",
                         copyArrows = TRUE,
                         geneAnnotation=geneAnnotation,
                         genomeAnnotation=genomeAnnotation)
...
plotPDF(p1,p2, name = "QC-Sample-FragSizes-TSSProfile.pdf", ArchRProj = projHemeNew, addDOC = FALSE, width = 5, height = 5)
##save ArchRProject
...
projHemeNew <- addDoubletScores(
    ...
    LSIMethod = 1)
======================================================================================================
##plot the Tss enrichment rideges and violin for every Sample
p3 <- plotGroups(
...
) + geom_hline(yintercept = 3.5, lty = "dashed") + geom_vline(xintercept = 3, lty = "dashed")
...
projHemeNew <- addIterativeLSI(ArchRProj = projHemeNew,
                            ...
                            name = "IterativeLSI",
                            iterations = 2,
                            clusterParams = list(resolution = c(0.4),
                                                 sampleCells =12790,
                                                 n.start = 10),
                            varFeatures = 30000,
                            dimsToUse = 1:30,
...
projHemeNew <- addUMAP(ArchRProj = projHemeNew,
                    reducedDims = "IterativeLSI",
                    name = "UMAP",
                    nNeighbors = 10, minDist = 0.5, force = TRUE,
                    metric = "cosine",
                    threads=45)
...
plotPDF(p1,p2 ,name = "Embedding_colorby_cellColData_UMAP_(cellColData and Sample).pdf", 
        ArchRProj = projHemeNew, 
        addDOC = FALSE, 
        width = 4, 
        height = 4)
...
seRNA <- readRDS("/md01/changjy/data/pbmc/scRNAseq/scRNA-Hematopoiesis-Granja-2019.rds")
seRNA

projHemeNew<-loadArchRProject("/md01/changjy/data/pbmc")
colnames(colData(seRNA))
table(colData(seRNA)$BioClassification)
projHemeNew <- addGeneIntegrationMatrix(
    ArchRProj = projHemeNew , 
...
###整合scRNAseq数据(有监督)
cM <- as.matrix(confusionMatrix(projHemeNew$Cluster, projHemeNew$predictedGroup_Un))
preClust <- colnames(cM)[apply(cM, 1 , which.max)]
cbind(preClust, rownames(cM))

unique(unique(projHemeNew$predictedGroup_Un))
...
groupList <- SimpleList(
    TNK = SimpleList(
        ATAC = projHemeNew$cellNames[projHemeNew$Cluster %in% clustTNK],
        RNA = rnaTNK
    ),
    NonTNK = SimpleList(
        ATAC = projHemeNew$cellNames[projHemeNew$Cluster %in% clustNonTNK],
        RNA = rnaNonTNK
    )    
)
projHemeNew <- addGeneIntegrationMatrix(
...
plotPDF(p1,p2, name = "Plot-UMAP-RNA-Integration.pdf", ArchRProj = projHemeNew, addDOC = FALSE, width = 5, height = 5)
saveArchRProject(ArchRProj = projHemeNew, load = TRUE,threads=40,overwrite=TRUE)
======================================================================================================
...
# markerGenes=c("GFAP","NFIA","SOX9","ATP13A4","CBS","CAMSAP1")  ##Ast

# markerGenes=c("PRL","TGAS113E22.1","TNFAIP8","HSPB1") ##lac  
# markerGenes=c("RLN3","HPGDS","CARTPT")  ##Gona
# markerGenes=c("S100A10","S100A6","S100A11","WNT5A")  ##FS
# markerGenes=c("TSHB","CHGA","CHGB","DIO2") ##Thy
# markerGenes=c("IGFBP7", "CFD", "TFPI2", "PODXL", "APOLD1", "RAMP2")  ##Endo
# markerGenes=c("PECAM1" ,"CD31" ,"RGS5" ,"ABCB1A","ITIH5")

# markerGenes=c("GAD1", "SLC32A1",     "GAD2",    "SLC6A1", "GABBR2" ,"GADD45B"   )  ##GABA
# markerGenes=c("SOX2")  ##stem cell

markerGenes=c("POU1F1",  "NR5A1" ,"POMC-A", "GH", "TSHB", "PRL","PECAM1") ##EC
markerGenes=c("PDGFRA","VIM","CDH2")
markerGenes=c("OTX2", "LHX2", "COL25A1") #posterior pituitary pituicyte cells
markerGenes=c( "PTPRC")   ##immune cell
markerGenes=c("RBC", "HBQ1")  #red cells

markerGenes=c("SOX2","HES1")

markerGenes=c("PITX1","OTX2","PDGFRA","PECAM1","PTPRC","MKI67")
p=plotEmbedding(
    ArchRProj = projHemeNew, 
...
plotPDF(p, name = "Plot-ALL_marker_gene.pdf", ArchRProj = projHemeNew, addDOC = FALSE, width = 5, height = 5)
...
markerGenes <- c("GZMB","KLRD1","NKG7",

"CD14","CD68","ITGAM",  ##Monocytes

"CST3","CLEC4C", "NRP1", ##DC cell

"CD79A","CD79B","MS4A1", #B cell

"CD19", "CD38", #Plasma cell

"CD3D", "CD3G", "CD8A","IL2RA") ##T cell
...
projHemeNew <- addGroupCoverages(ArchRProj = projHemeNew, groupBy = 'subtype') 
=================================================================================================================================
...
p <- plotBrowserTrack(
    ArchRProj = projHemeNew, 
    groupBy = "Cluster", 
    geneSymbol = markerGenes,          ## Interested gene list
...
plotPDF(plotVarDev, name = 'Variable-Motif-Deviation-Scores', width = 5, hright =5, ArchRProj = projHemeNew, addDOC = F)
