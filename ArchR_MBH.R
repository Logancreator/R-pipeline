...
projHemeNew<-loadArchRProject("/md01/changjy/data/Goose/snATAC_ME_new_filter")
#projHemeNew<-loadArchRProject("/md01/changjy/data/Goose/snATAC_PT_new")
...
gtf <- read.table("/md01/changjy/data/Goose/Goose_bulk/ref/ensemble/Anser_cygnoides_ensemble/genes/genes.gtf.gz", sep="\t")
gtf_sub <- gtf[which(gtf[, 3] == "genes"),]
genes <- GRanges(seqnames=Rle(gtf_sub[,1]), ranges=IRanges(gtf_sub[, 4], end=gtf_sub[, 5]), strand=Rle(strand(gtf_sub[, 7])), 
 gene_id=substr(gsub(".*gene_id ", "", gtf_sub[, 9]),1,18), symbol=substr(gsub(".*gene_id ", "", gtf_sub[, 9]),1,18))
...
#Import the file
inputfiles<-c("/md01/changjy/data/Goose/LBFC20210423/file_1/1E-ME/outs/anno_fragment_del.tsv.gz",

    "/md01/changjy/data/Goose/LBFC20210423/file_2/2-ME/outs/anno_fragment_del.tsv.gz",

    "/md01/changjy/data/Goose/LBFC20210423/file_3/3E-ME/outs/anno_fragment_del.tsv.gz",

    "/md01/changjy/data/Goose/LBFC20210423/file_4/4ME/outs/anno_fragment_del.tsv.gz")
samplenames<-c("1E-ME","2-ME","3E-ME","4ME")

# ##test
# inputfiles<- "/md01/changjy/data/Goose/LBFC20210423/file_3/3E-ME/outs/anno_fragment_del.tsv.gz"
# samplenames<-"3E-ME"
# #Import the file
# # inputfiles<-c(
# #     "/md01/changjy/data/Goose/LBFC20210423/file_1/1P-plus/outs/new.fragments.tsv.gz",
# #     "/md01/changjy/data/Goose/LBFC20210423/file_2/2-P/outs/new.fragments.tsv.gz",
# #     "/md01/changjy/data/Goose/LBFC20210423/file_3/3E-P/outs/new.fragments.tsv.gz",
# #     "/md01/changjy/data/Goose/LBFC20210423/file_4/4P/outs/new.fragments.tsv.gz")
# # samplenames<-c("1P-plus","2-P","3E-P","4P")
...
projHemeNew <- ArchRProject(ArrowFiles = ArrowFiles,
                         outputDirectory = "/md01/changjy/data/Goose/snATAC_ME_new",
                         copyArrows = TRUE,
                         geneAnnotation=geneAnnotation,
                         genomeAnnotation=genomeAnnotation)
...
saveArchRProject(ArchRProj = projHemeNew, outputDirectory="/md01/changjy/data/Goose/snATAC_ME_new_filter",load = TRUE,threads=40,overwrite=TRUE)
...
projHemeNew <- addUMAP(ArchRProj = projHemeNew,
                    reducedDims = "IterativeLSI",
                    name = "UMAP",
                    nNeighbors = 10, minDist = 0.6, force = TRUE,
                    metric = "cosine",
                    threads=45)
...
write.table(as.data.frame(markerList$C4),"/md01/changjy/data/Goose/snATAC_ME_new/markerList$C4.txt",sep="\t",quote=F)
write.table(as.data.frame(markerList$C9),"/md01/changjy/data/Goose/snATAC_ME_new/markerList$C9.txt",sep="\t",quote=F)
write.table(as.data.frame(markerList$C8),"/md01/changjy/data/Goose/snATAC_ME_new/markerList$C8.txt",sep="\t",quote=F)
...
# library("Rgraphviz")
# library("pathview")
# # 搜寻 OrgDb（比如我想查找物种 Anser cygnoides）
# hub <- AnnotationHub::AnnotationHub()
# query(hub,"Anser cygnoides")
# OrgDb <- hub[["AH86240"]]
# data <- as.character(markerList$C17$name)
# data_id <- mapIds(x = OrgDb,keys = data,keytype = "SYMBOL",column = "ENTREZID")
# geneID <- bitr(data_id, fromType = "SYMBOL", 
#                  toType = c("ENTREZID"),
#                  OrgDb =OrgDb) 
# erich.go.BP <- enrichGO(gene=geneID$ENTREZID,OrgDb = OrgDb,keyType = "SYMBOL",ont = "ALL",pvalueCutoff = 0.01,qvalueCutoff = 0.05)

##Set the markerGenes and plot them

markerGenes=c("VIM","ASCL1","NEUROG2","TUBB3",
"SLC17A6","PDGFRA","MEGF11","MBP","FOXJ1",
"GFAP","CAMSAP1","NEUROD1", "NEUROG1","DNAH11","ACTA2",
"CX3CR1","PTGDS",
"FEZF1","ADCYAP1",
"GAD1","GAD2","C1QA","C1QB","PROM1A","SNAP25", "TUBB3",
"ELAVl2",
"SNAP25","SYT1",
"NR2F2",
"C1QA", "CX3CR1",
...
"OLIG2",
"TOP2A",
"MRC1","FYN","MYH11","CCDC153","RX1",
"AGT",
"TH","CRABP1","AGRP","POMC-A"
,"GNIH","NPY","TACR3","ADCYAP1","VIPR2","C1QC","ITM2A","DCN","CD9","HDC","TM4SF1","ERMN",
"IDO2","PAX7","GRIN1","GAD1","GAD2","VCAN", "LHFPL3","SLC16A9", "COLEC12","VCAN","TNR","ADAMTS4","CHST2","CHST3","CHST5", "COL23A1", "SLC16A2", "LHX2", "PTN")
p=plotEmbedding(
    ArchRProj = projHemeNew, 
    colorBy = "GeneScoreMatrix", 
    name =  unique(markerGenes), 
    embedding = "UMAP",
...
markerGenes=c("FOLH1","GOT1","GRIN1","GRIN2B","HDLBP","SLC17A8","GLS","SLC1A1","SLC14A2","SLC1A6","SLC17A6")
markerGenes=c("ABAT","GABBR1","GABBR2","GAD1","GAD2","PPP1R1B","SLC6A1","SLC32A1","SLC6A13")
markerGenes=c("CACNA1E", "CACNA2D1", "KCNQ3",  "RYR3" ,"CACNB2","PAX7","PAX3","PAX7" ,"CNKSR3","MEIS2")
...
plotPDF(P, name = "Plot-GLU_marker_gene.pdf", ArchRProj = projHemeNew, addDOC = FALSE, width = 5, height = 5)

markerGenes=c("S100B", "S100A6", "S100A1",
"S100A11","IGFBP7","CFD","HBAD","CRIP1","IFI30")

markerGenes=c("Pou1f1","Prl","Gh","Ghrhr","Tshb","Top2a","Mki67",
"Pomc-a","Pax7","Crhr1" ,"Sox2","Vim","Fstl1","Col1a1","Ctss")
markerGenes=c( "RLN3","HPGDS", "CPLX1",  "HEXB", "NR5A1")
pdf("Plot-TEST_marker_gene.pdf")
markerGenes=c("SOX2","POMC","PRL","TSHB","ALDH1A2","NR3C1","DLK1","OLFM1","DIO2")
p=plotEmbedding(
    ArchRProj = projHemeNew, 
    colorBy = "GeneScoreMatrix", 
...
#####################
projHemeNew$subtype[projHemeNew$subtype%in%"C1"]="oligodendrocyte"
projHemeNew$subtype[projHemeNew$subtype%in%"C2"]="oligodendrocyte precursor"
projHemeNew$subtype[projHemeNew$subtype%in%c("C3","C4")]="astrocyte"
projHemeNew$subtype[projHemeNew$subtype%in%c("C5","C6")]="ependymocyte"
projHemeNew$subtype[projHemeNew$subtype%in%"C7"]="tanycyte/microglial"
projHemeNew$subtype[projHemeNew$subtype%in%"C8"]="microglial"
projHemeNew$subtype[projHemeNew$subtype%in%"C9"]="endothelial"
projHemeNew$subtype[projHemeNew$subtype%in%c("C10","C11","C13")]="neuron"
projHemeNew$subtype[projHemeNew$subtype%in%"C12"]="unknown"


##重新画UMAP
df = data.frame(projHemeNew@embeddings$UMAP$df, projHemeNew$subtype)
celltype = projHemeNew$subtype
p=ggplot(df, aes(x = IterativeLSI.UMAP_Dimension_1, y = IterativeLSI.UMAP_Dimension_2, color =projHemeNew.subtype)) +
  geom_point(size=0.05,alpha=1) + 
  theme_bw() + 
  #scale_color_gradient(low = "blue",high = "red")+
  scale_color_manual(values = c("#ffcbcb","#0072B5FF","#FCF5B3","#E18727FF","#21DB00","#66E1E6",
   "#20854EFF","#1f441e","#7876B1FF","hotpink",  "seashell3")) + 
   guides(color=guide_legend(override.aes=list(size=4)))+
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ggtitle("Celltype")+
        labs(x="UMAP_1",y="UMAP_2",
       title = "A UMAP visualization of the MBH scATAC-seq dataset")
p
ggsave(p,filename = "umap.pdf")
...
df = data.frame(projHemeNew@embeddings$UMAP$df, projHemeNew$Cluster)
celltype = projHemeNew$Cluster
df$TSS=projHemeNew$TSSEnrichment
# pdf("../ArchR/UMAP-all-NCM-cell-type-split-IC-size0.4-alpha0.5.pdf")
# ggplot(df, aes(x = IterativeLSI.dim8.UMAP_Dimension_1, y = IterativeLSI.dim8.UMAP_Dimension_2, color = celltype)) +
...
#         panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+ggtitle("Celltype")
# dev.off()
ggplot(df, aes(x = IterativeLSI.UMAP_Dimension_1, y = IterativeLSI.UMAP_Dimension_2, color = TSS)) +
  geom_point(size=0.4,alpha=0.5) + 
  theme_bw() + 
  scale_color_gradient(low = "blue",high = "red")+
 # scale_color_manual(values = c("#ffcbcb","#0072B5FF","#FCF5B3","#E18727FF","#21DB00","#66E1E6",
